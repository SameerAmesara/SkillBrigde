...
import { Avatar, Box, Button, Card,  CardActions, CardContent, CircularProgress,   MenuItem, Pagination, Select, TextField, Typography, useMediaQuery } from '@mui/material';
...
const AdvanceNetworkPage: React.FC = () => {
    ...
    const [filterByMyConnections, setFilterByMyConnections] = useState<boolean>(false);
    const [filterByRequestSent, setFilterByRequestSent] = useState<boolean>(false);
    const [filterByRequestReceived, setFilterByRequestReceived] = useState<boolean>(false);
    ...
    useEffect(() => {
        const fetchUserConnections = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:8000/networking/userconnections`, {
                    params: {
                        uid: uid
                    }
                });
    ...
    const fetchData = async (
        ...
        try {
            ...
            if (filterByMyConnections) {
                filteredUsers = users.filter((user: any) => userConnections.myConnections.includes(user.uid));
            } else if (filterByRequestSent) {
                filteredUsers = users.filter((user: any) => userConnections.requestSent.includes(user.uid));
            } else if (filterByRequestReceived) {
                filteredUsers = users.filter((user: any) => userConnections.requestReceived.includes(user.uid));
            }
    ...
    useEffect(() => {
        fetchData(pageNumber, filterByMyConnections, filterByRequestSent, filterByRequestReceived);
        return () => {
        };
                ...
                <Box>
                    <Box sx={{ display: 'flex', justifyContent: 'center', marginBottom: '20px' }}>
                        ...
                        >
                            ...
                            <MenuItem value="myconnections" onClick={handleMyConnectionsClick}>My Connections</MenuItem>
                            <MenuItem value="myconnections" onClick={handleRequestSentClick}>Connection Requests Sent</MenuItem>
                            <MenuItem value="myconnections" onClick={handleRequestReceivedClick}>Connections Requests Received</MenuItem>
                            <MenuItem value="myconnections" onClick={ShowAll}>Show All</MenuItem>
                            {/* Add more sorting options if needed */}
                        </Select>
                    </Box>
                    {paginatedUsers.length > 0 &&
                        paginatedUsers.map((userGroup: any[], index: number) => (
                            <Box key={index} sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '20px' }}>
                                {userGroup.map((user: any) => (
                                    <Card key={user.uid} sx={{ width: evenMoreSmall ? '60%' : isMobileSize ? '35%' : '22%', margin: '16px', padding: '2%' }}>                                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100%' }}>
                                        <Avatar src={user.image} sx={{ width: 140, height: 140 }} />
                                        <CardContent>
                                            <Typography gutterBottom variant="h5" component="div" align="center">
                                                {user.firstName} {user.lastName}
                                            </Typography>
                                            <Typography variant="body2" color="text.secondary" align="center">
                                                {user.profession ? user.profession : 'Profession not available '}
                                            </Typography>
                                            <Typography variant="body2" color="text.secondary" align="center">
                                                {user.companyName ? user.companyName : 'Company not available'}
                                            </Typography>
                                        </CardContent>
                                        <CardActions sx={{ justifyContent: 'center' }}>
                                            <Button variant="outlined" size="small" onClick={() => handleSendConnection(user.uid, user.flag)}>
                                                {user.flag === 'f' && 'Send Connection Request'}
                                                {user.flag === 'rs' && 'Request Sent, Click Again to Cancel'}
